...
static GRect rect_text_steps = {{ -7, -5},{ 200, 200 } };
static GRect rect_text_sleep = {{ -7, 112},{ 200, 200 } };
static GRect rect_text_date = {{34, 57},{ 200, 200 } };
static GRect rect_text_bat = {{-56, 59},{ 200, 200 } };

# else  

static GRect rect_text_steps = {{ -26, 7},{ 200, 200 } };
static GRect rect_text_sleep = {{ -26, 106},{ 200, 200 } };
static GRect rect_text_date = {{14, 57},{ 200, 200 } };
static GRect rect_text_bat = {{-60, 57},{ 200, 200 } };
  ...
  if (is_ready){
    ...
    if (!is_charging){
      if(battery_level==100)
        snprintf(battery_buffer, sizeof(battery_buffer), "F");
      else
        snprintf(battery_buffer, sizeof(battery_buffer), "%i",(int)battery_level);
    }
    else
    ...
    graphics_context_set_stroke_color(ctx, color_bubles);
...
static void createdatetext(void){ 
  ...
  if (is_photo_shooting){ 
    snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "W12");
    return; 
  }

  if (strcmp(pebble_Lang, "en_US") == 0)
  {
    snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "%s%i", weekdayLangEn[s_last_time.wday],s_last_time.day);
    return; 
  }

  if (strcmp(pebble_Lang, "fr_FR") == 0)
  {
    snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "%s%i", weekdayLangFr[s_last_time.wday],s_last_time.day);
    return;
  }

  if (strcmp(pebble_Lang, "de_DE") == 0)
  {
    snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "%s%i", weekdayLangGe[s_last_time.wday],s_last_time.day);
    return;
  }

  if (strcmp(pebble_Lang, "es_ES") == 0)
  {
    snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "%s%i", weekdayLangSp[s_last_time.wday],s_last_time.day);
    return;
  }
  // Par defaut, anglais
  snprintf(s_last_time.datetext, sizeof(s_last_time.datetext), "%s%i", weekdayLangEn[s_last_time.wday],s_last_time.day);
...
static void init() {
  ...
  text_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_BINARY_50));
  #else

  text_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_FONT_BINARY_40));
  ...
  if(persist_exists(KEY_VIBRATION)&& persist_exists(KEY_MN_MARKS)&& persist_exists(KEY_FIVE_MARKS)&&
    ...
    color_text=GColorDukeBlue;
    color_bg=GColorWhite;
    color_bubles=GColorBlack;
